...
/**
     ...
     *
     * The class operates on @ref mist::ring_finding::Hit — a plain POD struct
     * that the caller populates from their detector-specific Hit representation.
     *
     * ### Two-phase workflow
     *  1. **Setup** (once per run / geometry change) — call
     *     @ref mist::ring_finding::HoughTransform::build_lut with the
     *     channel-to-position map to pre-compute which accumulator cells each
     *     LUT key votes for at every candidate radius.
     *  2. **Per-event processing** — call
     *     @ref mist::ring_finding::HoughTransform::find_rings with a vector of
     *     @ref mist::ring_finding::Hit. The method votes, finds the best peak,
     *     removes the tagged
     ...
     * auto rings = ht.find_rings(hits, {.threshold_fraction = 0.3f, .min_hits = 5});
     * @endcode
     */
...
/**
     ...
     * a caller can leave any subset unspecified. With C++20 designated
     * initialisers the call site is self-documenting:
     *
     * @code{.cpp}
...
public:
    ...
    /**
         ...
         *  3. Collect hits within @c opts.collection_radius of the ring arc.
         ...
         * @param opts  Tuning parameters; see @ref FindRingsOptions for
         *              the full field list (threshold_fraction, min_hits,
         *              min_active, max_rings, collection_radius,
         *              aggregation_window_cells).
         * @return      Vector of @ref RingResult sorted in descending
         *              @c peak_votes order. Callers may rely on
         *              @c rings[0] being the strongest candidate even
         *              though the extraction order itself is "first ring
