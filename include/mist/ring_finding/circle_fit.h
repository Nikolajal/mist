...
namespace mist::ring_finding
{
    ...
    namespace detail
    {
        ...
        [[nodiscard]] inline double taubin_eta(const circle_moments &m)
        {
            ...
            for (int i = 0; i < 50; ++i)
            {
                const double yold = y;
                y = A0 + x * (A1 + x * (A2 + x * A3));
                if (std::fabs(y) > std::fabs(yold)) break;
                const double dy = A1 + x * (A22 + x * A33);
                if (dy == 0.0) break;
                const double xold = x;
                x = xold - y / dy;
                if (std::fabs((x - xold) / (x != 0.0 ? x : 1.0)) < 1e-12) break;
                if (x < 0.0) x = 0.0;
            }
            return x;
        }
                                             ...
                                             circle_method method = circle_method::kasa)
    {
        ...
        m.Mxx *= inv_n; m.Myy *= inv_n; m.Mxy *= inv_n;
        m.Mxz *= inv_n; m.Myz *= inv_n; m.Mzz *= inv_n;
        m.Mz = m.Mxx + m.Myy;
        ...
        switch (method)
        {
        case circle_method::taubin: eta = detail::taubin_eta(m); break;
        case circle_method::pratt:  eta = detail::pratt_eta(m);  break;
        case circle_method::kasa:   eta = 0.0;                   break;
        }
        if (!std::isfinite(eta))
            return result;
