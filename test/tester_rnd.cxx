...
{
    // As of v1.0.0 (mist:D-01) the inverted-bounds case is:
    //   - debug builds: assert(start <= end) fires (precondition violation)
    //   - release builds: arguments silently swapped (legacy 0.x behaviour
    //     retained as a courtesy; not guaranteed past 1.x)
    // The test exercises only the release branch; debug builds skip it so
    // ctest stays green under -DCMAKE_BUILD_TYPE=Debug.
#ifdef NDEBUG
    mist::Rnd r(3);
