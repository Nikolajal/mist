...
void test_format_overloads()
{
    capture_streams cap;
    mist::logger::set_min_level(mist::logger::LevelTag::Debug);

    mist::logger::info("fitted {} spectra", 3);
    mist::logger::debug("x={}, y={}", 1, 2.5);
    mist::logger::log(mist::logger::LevelTag::Warning, "code {}", 42);
    // Zero-arg call still resolves to the plain (string_view, bool) overload.
    mist::logger::info("plain literal");

    const std::string out = cap.cout_buf.str();
    const std::string err = cap.cerr_buf.str();
    CHECK(out.find("fitted 3 spectra") != std::string::npos);
    CHECK(out.find("x=1, y=2.5") != std::string::npos);
    CHECK(out.find("plain literal") != std::string::npos);
    CHECK(err.find("code 42") != std::string::npos);
}

void test_done_output()
{
    capture_streams cap;
    mist::logger::done("all good");
    mist::logger::done("{} of {}", 2, 5);

    const std::string out = cap.cout_buf.str();
    CHECK(out.find("all good") != std::string::npos);
    CHECK(out.find("2 of 5") != std::string::npos);
    // done() bypasses the level filter — visible even at the most restrictive
    // setting.
    mist::logger::set_min_level(mist::logger::LevelTag::Error);
    mist::logger::done("still shown");
    CHECK(cap.cout_buf.str().find("still shown") != std::string::npos);
    mist::logger::set_min_level(mist::logger::LevelTag::Debug);
}
...
{
    {
        capture_streams cap; // forces tty on, colour off
        {
            mist::logger::ScopedCoutToMist redirect;
            std::cout << "root minimiser chatter\n";
            std::cout << "second line\n";
        }
        // After the scope the routed lines have been emitted through the
        // logger into the captured cout buffer.
        const std::string out = cap.cout_buf.str();
        CHECK(out.find("root minimiser chatter") != std::string::npos);
        CHECK(out.find("second line") != std::string::npos);
        // Stream buffer restored: a direct write still lands in the capture.
        std::cout << "after restore";
        CHECK(cap.cout_buf.str().find("after restore") != std::string::npos);
    }

    // No-op when not a TTY: the cout buffer is left untouched for the guard's
    // lifetime.
    {
        capture_streams cap;
        mist::logger::set_tty(false);
        std::streambuf *before = std::cout.rdbuf();
        {
            mist::logger::ScopedCoutToMist redirect;
            CHECK(std::cout.rdbuf() == before); // unchanged -> no-op
        }
        CHECK(std::cout.rdbuf() == before);
    }
}
