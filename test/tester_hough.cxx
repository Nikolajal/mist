...
void test_find_rings_before_build_returns_empty()
{
    ...
    std::vector<mist::ring_finding::Hit> hits;
    const auto rings = ht.find_rings(hits, {.threshold_fraction = 0.3f, .min_hits = 5, .min_active = 5});
    CHECK(rings.empty());
...
void test_find_rings_recovers_two_disjoint_rings()
{
    ...
    const auto rings = ht.find_rings(hits, {.threshold_fraction = 0.2f, .min_hits = 5, .min_active = 5, .max_rings = 2});
    CHECK(rings.size() == 2);
    if (rings.size() != 2)
        return;
...
void test_max_rings_cap()
{
    ...
    const auto rings = ht.find_rings(hits, {.threshold_fraction = 0.2f, .min_hits = 5, .min_active = 5, .max_rings = 1});
    CHECK(rings.size() <= 1);
}
...
void test_empty_hits_returns_no_rings()
{
    ...
    const auto rings = ht.find_rings(empty, {.threshold_fraction = 0.3f, .min_hits = 5, .min_active = 5});
    CHECK(rings.empty());
}
