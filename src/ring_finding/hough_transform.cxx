    ...
    for (int iR = 0; iR <= max_iR; ++iR)
    {
        ...
        for (int iy = 0; iy <= max_iy; ++iy)
        {
            ...
            for (int ix = 0; ix <= max_ix; ++ix)
            {
                ...
                // − for the back corner.
                const int sum =
                    sat_val(iR1, iy1, ix1) - sat_val(iR - 1, iy1, ix1) - sat_val(iR1, iy - 1, ix1) - sat_val(iR1, iy1, ix - 1) + sat_val(iR - 1, iy - 1, ix1) + sat_val(iR - 1, iy1, ix - 1) + sat_val(iR1, iy - 1, ix - 1) - sat_val(iR - 1, iy - 1, ix - 1);
                                                   ...
                                                   const ::mist::ring_finding::FindRingsOptions &opts)
{
    ...
    // Unpack opts into local names so the rest of the (long) body
    // touches no struct-accessor overhead in the hot inner loop and
    // reads identically to its pre-options-struct form.
    const float threshold_fraction = opts.threshold_fraction;
    const int min_hits = opts.min_hits;
    const int min_active = opts.min_active;
    const int max_rings = opts.max_rings;
    const float collection_radius = opts.collection_radius;
    ...
    const int window = std::max(1, opts.aggregation_window_cells);
