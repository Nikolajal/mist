                                ...
                                float r_min, float r_max, float r_step, float cell_size)
{
    ...
    x_min_ -= r_max; x_max_ += r_max;
    y_min_ -= r_max; y_max_ += r_max;
    ...
    mist::logger::info(
        "(hough_transform::build_lut) LUT built: "
        + std::to_string(lut_.size())   + " keys, "
        + std::to_string(r_bins_.size()) + " R bins, grid "
                                                     ...
                                                     float collection_radius)
{
           ...
           static_cast<int>(active_indices.size()) >= min_active)
    {
        ...
        if (static_cast<int>(ring.hit_indices.size()) < min_hits) break;
        ...
        active_indices.erase(
            std::remove_if(active_indices.begin(), active_indices.end(),
                           [&ring_set](int idx) { return ring_set.count(idx) > 0; }),
            active_indices.end());

        if (static_cast<int>(active_indices.size()) < threshold) break;
    }
