 ...
 *    state.
 *  - The global @ref mist::logger::AnchorObject registry mutex is the outer
 *    lock; the multi-bar's @c mutex_ is the inner lock.  Subtask update paths
 *    take the multi-bar mutex first, mutate state, RELEASE, then call the
 *    registry-locked anchor operations.  @ref mist::logger::AnchorObject::render_line
 *    acquires the multi-bar mutex briefly while the registry mutex is already
 *    held by the caller.
